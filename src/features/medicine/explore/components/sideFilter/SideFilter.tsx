...
import { Acquisition, Golongan, type Pharmacy } from "@/types/semuaNgerapiinyaNtar"
...
const SideFilter: FC<SideFilterProps> = ({
  ...
  const hasActiveFilters = selectedAquisition || selectedGolongan || selectedCategories.length > 0

  return (
    <div className={`space-y-6 ${className}`}>
      <PharmacySelector pharmacy={sleectedPharmacy} setPharmacy={onPharmacyChange} />
      ...
      <Card className="shadow-sm border-0 bg-white/80 backdrop-blur-sm">
        ...
        <CardContent className="pt-0">
          <Accordion type="multiple" defaultValue={["acquisition", "golongan", "categories"]}>
            {/* Acquisition Filter */}
            <AccordionItem value="acquisition" className="border rounded-lg mb-4">
                  ...
                  {Object.values(Acquisition).map((aqu, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => onAcquisitionChange(checked ? aqu : "")}
                      />
                      <Label htmlFor={`acquisition-${aqu}`} className="text-sm font-normal cursor-pointer flex-1">
                        {aqu
                          .split("_")
                  ...
                  {Object.values(Golongan).map((group, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => onGolonganChange(checked ? group : "")}
                      />
                      <Label htmlFor={`golongan-${group}`} className="text-sm font-normal cursor-pointer flex-1">
                        {group
                          .split("_")
                          .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
                          .join(" ")}
